...
Las siete páginas HTML y la hoja de estilos se validaron contra los servicios del W3C. Las únicas advertencias corresponden a propiedades modernas (\lstinline|backdrop-filter|, \lstinline|color-mix|, \lstinline|oklch|) interpretadas correctamente por navegadores actuales.
...
Se recomienda adjuntar capturas de las siete páginas en los tres \emph{viewports} principales (móvil 375~px, tablet 768~px, desktop 1280~px), incluyendo: la página de inicio con el menú activo, el formulario de cita mostrando la calculadora de costo, el modal de confirmación, el menú hamburguesa abierto en móvil y el \emph{dashboard} con sus gráficos.
